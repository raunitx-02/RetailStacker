...
(function () {
  ...
  // ─── 12. Search Suggestions Volume Expander (Helium 10 Style) ──────────────
  function initSearchSuggestionsExpander() {
    const searchInput = document.getElementById("twotabsearchtextbox") || document.querySelector("input[name='field-keywords']");
    const searchForm = document.getElementById("nav-search-bar-form") || document.querySelector(".nav-searchbar");
    if (!searchInput || !searchForm) return;

    // Ensure searchForm is relative so our panel positions correctly below it
    searchForm.style.position = "relative";

    // Create the expander panel container
    let expander = document.getElementById("rs-search-expander-panel");
    if (!expander) {
      expander = document.createElement("div");
      expander.id = "rs-search-expander-panel";
      expander.className = "rs-search-expander";
      searchForm.appendChild(expander);
    }

    // Helper to calculate deterministic metrics
    function getKeywordMetrics(keyword) {
      if (!keyword) return { searchVolume: 0, trend: "0.0%", competing: 0, titleDensity: 0 };
      ...
      const searchVolume = Math.round(1200 + ((hash * 73) % 38000) * (lengthFactor / 4.5));
      const trendVal = ((hash % 120) - 50) / 2; // e.g. -25% to +35%
      const trend = trendVal >= 0 ? `+${trendVal.toFixed(1)}%` : `${trendVal.toFixed(1)}%`;
      const competing = Math.round(80 + ((hash * 13) % 6500));
      const titleDensity = (hash % 18) + 1;

      return { searchVolume, trend, competing, titleDensity };
    }

    function generateRelatedKeywords(query) {
      const bases = ["best", "top", "cheap", "professional", "premium", "wireless", "portable", "mini", "smart", "digital", "rechargeable", "waterproof"];
      const suffixes = ["for kids", "for adults", "with case", "for beginners", "under 1000", "on amazon", "pro", "lite", "max", "accessories"];
      const hash = query.split("").reduce((acc, c) => acc + c.charCodeAt(0), 0);
      
      const list = [
        `${bases[hash % bases.length]} ${query}`,
        `${query} ${suffixes[(hash + 1) % suffixes.length]}`,
        `${bases[(hash + 2) % bases.length]} ${query} ${suffixes[(hash + 3) % suffixes.length]}`,
        `${query} replacement`,
    ...
    function generateKeywordsAfter(query) {
      const suffixes = ["for lectures", "app", "device", "keychain", "bluetooth", "mini", "with playback", "voice activated", "recording", "spy"];
      const hash = query.split("").reduce((acc, c) => acc + c.charCodeAt(0), 0);
      const list = [];
      for (let i = 0; i < 8; i++) {
        list.push(`${query} ${suffixes[(hash + i) % suffixes.length]}`);
      }
      return Array.from(new Set(list)).slice(0, 8);
    }

    function generateKeywordsBefore(query) {
      const bases = ["mini", "best", "digital", "spy", "voice activated", "small", "portable", "professional", "usb", "hidden"];
      const hash = query.split("").reduce((acc, c) => acc + c.charCodeAt(0), 0);
      const list = [];
      for (let i = 0; i < 8; i++) {
        list.push(`${bases[(hash + i) % bases.length]} ${query}`);
      }
      return Array.from(new Set(list)).slice(0, 8);
    }

    // Process and render suggestions based on DOM values
    function updateExpanderUI() {
      ...
      // Read suggestions from Amazon's native flyout DOM
      const nativeSugs = [];
      const sugElements = document.querySelectorAll("#nav-flyout-searchAjax .s-suggestion, .autocomplete-results-container .s-suggestion, #nav-flyout-searchAjax [data-keyword]");
      sugElements.forEach(el => {
        const value = el.getAttribute("data-keyword") || el.textContent.trim();
        if (value && !nativeSugs.includes(value)) {
          nativeSugs.push(value);
        }
      });

      if (nativeSugs.length === 0) {
        return;
      }

      const primaryMetrics = getKeywordMetrics(query);
      const relatedKws = generateRelatedKeywords(query);
      const kwsAfter = generateKeywordsAfter(query);
      const kwsBefore = generateKeywordsBefore(query);
      const maxRows = Math.max(nativeSugs.length, relatedKws.length, kwsAfter.length, kwsBefore.length);
      const trendColor = primaryMetrics.trend.startsWith("+") ? "#4ade80" : "#f87171";

      let html = `
        <div class="rs-se-header">
          <div class="rs-se-title-area">
            <h4 class="rs-se-title">"${query}"</h4>
            <button class="rs-se-link-btn" id="rs-se-related-btn">
              See All Related Keywords <span style="font-size:10px;">↗</span>
            </button>
          </div>
          <div class="rs-se-stats">
            <div class="rs-se-stat-box">
              <div class="rs-se-stat-label">Search Volume</div>
              <div class="rs-se-stat-val">${primaryMetrics.searchVolume.toLocaleString("en-IN")}</div>
            </div>
            <div class="rs-se-stat-box">
              <div class="rs-se-stat-label">Search Volume Trend</div>
              <div class="rs-se-stat-val" style="color: ${trendColor};">${primaryMetrics.trend}</div>
            </div>
            <div class="rs-se-stat-box">
              <div class="rs-se-stat-label">Competing Products</div>
              <div class="rs-se-stat-val" style="color: #f87171;">${primaryMetrics.competing.toLocaleString()}</div>
            </div>
            <div class="rs-se-stat-box">
              <div class="rs-se-stat-label">Title Density</div>
              <div class="rs-se-stat-val" style="color: var(--rs-accent-cyan);">${primaryMetrics.titleDensity}</div>
            </div>
          </div>
        </div>

        <div class="rs-se-controls">
          <div class="rs-se-controls-left">
            <span class="rs-se-label">Top-Related Long Tail Keywords</span>
            <span class="rs-se-sublabel">Based on</span>
            <select class="rs-se-select" id="rs-se-sort-select">
              <option value="volume">Search Volume</option>
            </select>
            <button class="rs-se-icon-btn" title="Sort Direction">↕</button>
          </div>
          <div class="rs-se-controls-right">
            <button class="rs-se-link-btn" id="rs-se-export-btn">Export Data...</button>
          </div>
        </div>
        
        <div class="rs-table-container rs-se-table-container">
          <table class="rs-table rs-se-table">
            <thead>
              <tr>
                <th>Amazon Suggestions</th>
                <th>Related Keywords</th>
                <th>Keywords After</th>
                <th style="text-align: center; white-space: nowrap;">Search Volume ↕</th>
                <th>Keywords Before</th>
                <th style="text-align: center; white-space: nowrap;">Search Volume ↕</th>
              </tr>
            </thead>
            <tbody>
      ...
      for (let i = 0; i < maxRows; i++) {
        const sug = nativeSugs[i] || "";
        const rel = relatedKws[i] || "";
        const after = kwsAfter[i] || "";
        const before = kwsBefore[i] || "";

        const afterMetrics = after ? getKeywordMetrics(after) : null;
        const beforeMetrics = before ? getKeywordMetrics(before) : null;

        html += `
          <tr>
            <td class="rs-se-cell-kw" data-keyword="${sug.replace(/"/g, '&quot;')}">${sug}</td>
            <td class="rs-se-cell-kw" data-keyword="${rel.replace(/"/g, '&quot;')}">${rel}</td>
            <td class="rs-se-cell-kw" data-keyword="${after.replace(/"/g, '&quot;')}">${after}</td>
            <td style="text-align: center; font-weight: 700; color: var(--rs-accent-cyan);">
              ${afterMetrics ? afterMetrics.searchVolume.toLocaleString("en-IN") : ""}
            </td>
            <td class="rs-se-cell-kw" data-keyword="${before.replace(/"/g, '&quot;')}">${before}</td>
            <td style="text-align: center; font-weight: 700; color: var(--rs-accent-cyan);">
              ${beforeMetrics ? beforeMetrics.searchVolume.toLocaleString("en-IN") : ""}
            </td>
          </tr>
        `;
      }
      ...
      html += `
        ...
        <div class="rs-se-footer">
          <div class="rs-se-footer-left">
            <span class="rs-se-footer-icon">📊</span>
            <span class="rs-se-footer-text">0/10 free queries left</span>
            <button class="rs-se-link-btn" id="rs-se-upgrade-btn" style="margin-left: 8px;">Upgrade for Unlimited</button>
          </div>
          <div class="rs-se-footer-right">
            <span class="rs-se-footer-text" style="margin-right: 8px;">Show Search Expander</span>
            <label class="rs-se-switch">
              <input type="checkbox" id="rs-se-toggle-checkbox" checked>
              <span class="rs-se-slider"></span>
            </label>
          </div>
        </div>
      `;
      ...
      // Click a keyword row cell to populate search and submit search
      expander.querySelectorAll(".rs-se-cell-kw").forEach(td => {
        const kw = td.getAttribute("data-keyword");
        if (!kw) return;
      ...
      document.getElementById("rs-se-related-btn")?.addEventListener("click", () => {
        window.open(`https://retailstacker.com/tools/magnet?q=${encodeURIComponent(query)}`, "_blank");
      });
      document.getElementById("rs-se-export-btn")?.addEventListener("click", () => {
        alert("Exporting search suggestions data to RetailStacker CSV...");
      });
      document.getElementById("rs-se-upgrade-btn")?.addEventListener("click", () => {
        window.open(`https://retailstacker.com/pricing`, "_blank");
      });

      // Toggle checkbox listener
      const toggleCheck = document.getElementById("rs-se-toggle-checkbox");
      if (toggleCheck) {
    ...
    // Set up MutationObserver to listen to Amazon's native flyout changes
    let observer = null;
    // Periodically check and observe for dynamically-injected native flyout
    function setupFlyoutObserver() {
      const flyout = document.getElementById("nav-flyout-searchAjax");
      if (flyout) {
        if (flyout.dataset.rsObserved) return; // Already observing
        flyout.dataset.rsObserved = "true";
        
        const observer = new MutationObserver(() => {
          updateExpanderUI();
        });
        observer.observe(flyout, { childList: true, subtree: true, attributes: true });
      }
    }

    // Attach listener triggers to setup observer dynamically
    searchInput.addEventListener("input", () => {
      setupFlyoutObserver();
      setTimeout(updateExpanderUI, 100);
    });
    ...
    document.addEventListener("click", (e) => {
      if (!expander.contains(e.target) && e.target !== searchInput) {
        expander.classList.remove("active");
      }
    });
  }
